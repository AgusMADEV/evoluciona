...
fetch(ruta_back+"?busca=productos&campo=Identificador&dato="+idproducto)                         // Construyo una peticion al servidor solo con el parametro que me interesa
...
function eliminarProducto(nombreProducto) {
    ...
    // Find the first occurrence of the product
    const index = productos.findIndex(producto => producto.nombre === nombreProducto);
    
    if (index !== -1) { 
        productos.splice(index, 1); // Remove only the first found occurrence
    }

    localStorage.setItem(clavealmacenaje, JSON.stringify(productos));
    console.log("Producto eliminado:", nombreProducto);

    listadoProductos(); // Refresh product list
    setEliminarEventos(); // Reattach delete event listeners
}